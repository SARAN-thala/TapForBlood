class RequestResponseMappingsController < ApplicationController
  ...
  def my_responses
    ...
    responded_users = filtered_request_responses.collect do |mapping|
  ...
  def my_requests
    ...
    all_other_active_requests = all_active_requests.reject { |req| req.user_id == user.id }
    all_other_active_requests.each do |req|
      requested_user = User.find(req.user_id)
      next unless blood_grouping[req.blood_group].include?(user.blood_group)
      distance = distance(user.latitude.to_f, user.longitude.to_f,
                          requested_user.latitude.to_f, requested_user.longitude.to_f)
      my_requests.push(request: req, user: requested_user) if distance < 100
    end
